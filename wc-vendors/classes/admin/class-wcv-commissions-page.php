...
class WCVendors_Commissions_Page extends WP_List_Table {
    ...
    public function column_default( $item, $column_name ) {
        ...
        switch ( $column_name ) {
            ...
            case 'product_id':
                ...
                $wcv_total_sales = $this->get_product_total_sales( $item->product_id );
                $product_url     = '';
                ...
                if ( '' !== get_the_title( $product_id ) ) {
                    $product_url = '<a href="' . admin_url( 'post.php?post=' . $product_id . '&action=edit' ) . '">' . get_the_title( $item->product_id ) . '</a> (<span title="' . get_the_title( $item->product_id ) . ' has sold ' . $wcv_total_sales . ' times total.">' . $wcv_total_sales . '</span>)';
                } else {
    ...
    public function extra_tablenav( $which ) {
        ...
        if ( 'top' === $which ) {
            ...
            // The export handlers require manage_options. Only show the buttons to users who can use them.
    ...
    public function mark_reversed( $ids = '' ) {
        ...
        $ids  = escape_array_for_in_operator( explode( ',', $ids ), true );
        $sql  = "UPDATE `{$wpdb->prefix}pv_commission` SET `status` = %s WHERE id IN";
    ...
    public function prepare_items() {
        ...
        if ( ! empty( $product_ids ) ) {
            $this->products_count = wcv_get_product_total_sales( $product_ids );
        }
    ...
    public function get_product_total_sales( $product_id ) {
        ...
        if ( ! empty( $this->products_count ) ) {
            if ( isset( $this->products_count[ $product_id ] ) ) {
                $total_sales = $this->products_count[ $product_id ];
            }
        }
        return $total_sales;
    }
