...
const Ja = 'modulepreload',
  ...
  Qa = M({
    ...
    setup(e) {
      const t = fr(() =>
          Ct(
            () => import('./components/Admin.a922f9f7.js'),
            [
        ...
        n = fr(() =>
          Ct(
            () => import('./components/Spinner.98b720f0.js'),
            [
              window.wcv_avp.pluginDirUrl +
...
const ns = e => typeof e == 'function',
  ...
  Cp = An(e => e.charAt(0).toUpperCase() + e.slice(1)),
...
const Io = Symbol('StyleContextKey'),
  ...
  ko = { cache: tt(), defaultCache: !0, hashPriority: 'low' },
  _t = () => {
    const e = ks()
    return ee(Io, J(v(v({}, ko), { cache: e })))
  },
  js = e => {
    const t = _t(),
      n = J(v(v({}, ko), { cache: tt() }))
...
function ll() {
  return (k = K > 0 ? nt(De, --K) : 0), Ie--, k === 10 && ((Ie = 1), Rt--), k
}
function Y() {
  return (k = K < Xo ? nt(De, K++) : 0), Ie++, k === 10 && ((Ie = 1), Rt++), k
}
function ve() {
  return nt(De, K)
}
...
function pl(e, t) {
  for (; Y() && e + k !== 47 + 10; ) if (e + k === 42 + 42 && ve() === 47) break
...
const le = (e, t) => new L(e).setAlpha(t).toRgbString(),
...
function Oe(e, t, n) {
  ...
  if (ot(r, 'validator') && je(r.validator)) {
    var u = B,
      c = []
    if (
      ((B = function(f) {
    ...
    setup(e, t) {
      ...
      const m = w(() => {
        ...
        re = w(() => {
          ...
          return v(v({}, ue), { theme: Kt, token: v(v({}, kt), N) })
        }),
        Xt = w(() => {
          var x, A
          let N = {}
...
j.mergeConfig = we
j.AxiosHeaders = Z
j.formToJSON = e => ua(h.isHTMLForm(e) ? new FormData(e) : e)
j.getAdapter = ha.getAdapter
j.HttpStatusCode = tp
j.default = j
const np = j,
  se = np.create({
...
const op = async e => (await se.get('/vendors', { params: e })).data,
  ip = async e => (await se.get(`/vendors/settings/${e}`)).data,
  ap = async e => (await se.get(`/vendors/settings/${e}`)).data,
  sp = async (e, t) =>
    (await se.post(`/vendors/settings/${e}`, { changes: t })).data,
  lp = async (e, t) => (await se.post(`/vendors/action/${e}/${t}`)).data,
  cp = async (e, t, n, r) =>
    (
      await se.post(`/vendors/action/${e}/${t}`, {
        customMessage: n || '',
  ...
  up = async (e, t, n, r) =>
    (
      await se.post('/vendors/bulk-action', {
        vendor_ids: e,
        action: t,
        customMessage: n || '',
        useCustomMessage: r || !1
  ...
  fp = async e => (await se.get(`/vendors/details/${e}`)).data,
  dp = {
    getVendors: op,
    getVendor: ip,
    getVendorDetails: fp,
    ...
    bulkVendorApproval: up
  },
  pp = window.wcv_avp.html_settings,
  mp = {
    plugins:
    ...
    routes: [
      {
        ...
        component: async () =>
          await Ct(
            () => import('./components/VendorTable.9b3bdb93.js'),
            [
              window.wcv_avp.pluginDirUrl +
      ...
      },
      {
        ...
        component: async () =>
          await Ct(
            () => import('./common/VendorEdit.465a2363.js').then(e => e.V),
            [
              window.wcv_avp.pluginDirUrl +
...
window.wcv_avp.registerComponent = hp
