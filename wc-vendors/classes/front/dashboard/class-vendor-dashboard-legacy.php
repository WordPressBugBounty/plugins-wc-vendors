...
class WCV_Vendor_Dashboard_Legacy {
        ...
        if ( isset( $_POST['wc-product-vendor-nonce'] ) ) {
            ...
            if ( isset( $_POST['pv_paypal'] ) && '' !== $_POST['pv_paypal'] ) {
                if ( ! is_email( $_POST['pv_paypal'] ) ) {
                    ...
                    update_user_meta( $user_id, 'pv_paypal', sanitize_email( $_POST['pv_paypal'] ) );
                }
            } else {
            ...
            if ( ! empty( $_POST['pv_shop_name'] ) ) {
                ...
                if ( ! empty( $users ) && $users[0]->ID !== $user_id ) {
                    ...
                    update_user_meta( $user_id, 'pv_shop_name', sanitize_text_field( $_POST['pv_shop_name'] ) );
    ...
    public function display_vendor_products( $atts ) {
        ...
        if ( ! is_user_logged_in() ) {
            ...
            printf(
                '%s <a href="%s">%s</a> to view this page.',
                esc_html__( 'Please', 'wc-vendors' ),
                esc_url( $login_url ),
                esc_html__( 'login', 'wc-vendors' )
            );
            return ob_get_clean();
        }
        ...
        wc_enqueue_js( self::wc_st_js( $provider_array ) );
